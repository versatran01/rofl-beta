...
TEST(Opencv, MatSetRoi) {
  cv::Mat x = cv::Mat::zeros(2, 2, CV_8UC1);
  EXPECT_EQ(MatSetRoi<uint8_t>(x, {0, 0, 2, 2}, 1), true);
  EXPECT_EQ(x.at<uint8_t>(0, 0), 1);
  EXPECT_EQ(MatSetRoi<uint8_t>(x, {2, 2, 4, 4}, 0), false);
}
